...
// map of paths for executing the tasks below
...
gulp.task('build'), ['todo'], function() {
    // Add later
    // add entry file and build using the webpack plugin.
    // The plugin requires webpack.config as an argument.
    // Stream result to the destination path.
}
...
// copy over required files to dist dir.
  gulp.task('copy', function() {
    return gulp.src(paths.toCopy, { base: 'client' })
      .pipe(gulp.dest(paths.dest));
  });
  
// watch files for new changes and execute build and copy tasks
